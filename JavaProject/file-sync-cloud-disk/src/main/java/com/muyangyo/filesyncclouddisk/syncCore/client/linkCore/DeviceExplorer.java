...
public class DeviceExplorer {
        ...
        while (isRunning) { // 使用 isRunning 来控制循环
            ...
            if (!connected) {
                ...
                for (String deviceInfo : devices) { //有设备在局域网返回了服务端信息
                    ...
                    // 判断是否是有效的服务端设备
                    if (checkServerDeviceId(deviceId, savedServerDeviceOriginId)) {
    ...
    private static boolean checkServerDeviceId(String deviceId, String savedServerDeviceOriginId) {
        if (deviceId.equals(savedServerDeviceOriginId)) {
            log.info("确认服务端设备ID正确");
            return true;
        }
        log.warn("确认服务端设备ID错误, 期望: [{}] , 实际: [{}]", savedServerDeviceOriginId, deviceId);
        return false;
    }
}